...
def clickedsend():
    ...
    msg['From']='GraceFul Heart Restaurant'
    msg['To']=textmail.get()
    content="\nThank you for visiting Graceful heart\nplease find the reciept attachment below"
    msg.set_content(content)
    pdf=FPDF()
    pdf.add_page()
    pdf.set_font("Times",size=20)
    fd=open(f"Bill Records\\{folder}\\{customer_name+customer_contact}.txt","r")
    for i in fd:
        pdf.cell(200,10,txt=i,ln=1,align='L')
    pdf.output(f"{customer_name+customer_contact}.pdf")
    with open(f"{customer_name+customer_contact}.pdf",'rb') as myfile:
        data = myfile.read()
        msg.add_attachment(data,maintype="application",subtype="pdf",filename=customer_name+'_receipt')
    with smtplib.SMTP_SSL('smtp.gmail.com',465) as server:
        server.login('miniproject123310@gmail.com','ydabikhfhwepigva')
        server.send_message(msg)
        tmsg.showinfo('Send Successfully','Message sent succesfully')
...
def bill_button_operation():
    ...
    if ans=="yes":
        ...
        labelmail=Label(bill,text='Enter Mail',font=('arial',16,'bold'),bg='#732CAF',fg='white')
        labelmail.pack()

        global textmail
        textmail = Entry(bill, font=('arial', 18, 'bold'), bd=7, width=40)
        textmail.pack()


        buttonsend=Button(bill,text='Send',font=('arial',14,'bold'),fg='white',bg='#732CAF',bd=3,padx=5,command=clickedsend)
        buttonsend.pack()
...
title_frame = Frame(root, bd=8, bg="#732CAF", relief=GROOVE)
title_frame.pack(side=TOP, fill="x")

title_label = Label(title_frame, text="** GRACEFUL HEART RESTAURANT **", 
                    font=("times new roman", 20, "bold"),bg = "#732CAF", fg="#d6baed", pady=5)
title_label.pack()


customer_frame = LabelFrame(root,text="Customer Details",font=("times new roman", 15, "bold"),
                            bd=8, bg="#732CAF",fg="#d6baed", relief=GROOVE)
customer_frame.pack(side=TOP, fill="x")

customer_name_label = Label(customer_frame, text="Name", 
                    font=("arial", 15, "bold"),bg = "#732CAF", fg="#d6baed")
...
menu_frame = Frame(root,bd=8, bg="#732CAF", relief=GROOVE)
menu_frame.place(x=0,y=125,height=670,width=684)

menu_label = Label(menu_frame, text="Menu", 
                    font=("times new roman", 20, "bold"),bg = "#732CAF", fg="#d6baed", pady=0)
menu_label.pack(side=TOP,fill="x")

menu_category_frame = Frame(menu_frame,bg="#732CAF",pady=10)
menu_category_frame.pack(fill="x")

combo_lable = Label(menu_category_frame,text="Select Type", 
                    font=("arial", 12, "bold"),bg = "#732CAF", fg="#d6baed")
...
item_frame = Frame(root,bd=8, bg="#732CAF", relief=GROOVE)
item_frame.place(x=680,y=125,height=230,width=852)

item_title_label = Label(item_frame, text="Item", 
                    font=("times new roman", 20, "bold"),bg = "#732CAF", fg="#d6baed")
item_title_label.pack(side=TOP,fill="x")

item_frame2 = Frame(item_frame, bg="#732CAF")
...
item_frame3 = Frame(item_frame, bg="#732CAF")
...
order_frame = Frame(root,bd=8, bg="#732CAF", relief=GROOVE)
order_frame.place(x=680,y=335,height=460,width=852)
...
order_tabel_frame.place(x=0,y=40,height=260,width=852)
